...
def enrich_recommendations(recs_df, user_id, algorithm_type):
    ...
    user_ratings = []
    explanations = []
    regions = []    # Neue Liste für RegionName
    grape_varieties = []  # Neue Liste für Grapes

    for _, row in recs_df.iterrows():
        # Standardwerte für Region und Grapes setzen
        region = 'Unknown Region'
        grapes = 'Unknown Grapes'
        
        # Suche den Wein anhand der Wein-ID
        ...
        # Region und Grapes zur jeweiligen Liste hinzufügen
        regions.append(region)
        grape_varieties.append(grapes)

        # Suche die Bewertung des Nutzers für den aktuellen Wein
        ...
        # Generiere die Erklärung basierend auf dem Algorithmustyp
    ...
    # Ergänze die Spalten zum ursprünglichen DataFrame
    recs_df['WineName'] = wine_names
    recs_df['Your Rating'] = user_ratings
    recs_df['RegionName'] = regions  # Liste der Regionen in DataFrame hinzufügen
    recs_df['Grapes'] = grape_varieties  # Liste der Grapes in DataFrame hinzufügen
...
def calculate_content_diversity(recommendations_df, content_columns=['RegionName', 'Grapes']):
    ...
    """
    # Überprüfe, ob alle benötigten Spalten in `recommendations_df` vorhanden sind
    ...
    # Kombiniere die Eigenschaften jeder Zeile, um den Inhalt jedes Weins zu repräsentieren
    ...
    # Filtere leere Inhalte heraus
    valid_contents = recommendations_df['combined_content'].replace('', pd.NA).dropna()

    if valid_contents.empty:
        return 0

    # Konvertiere die Inhalte in numerische Vektoren (TF-IDF)
    vectorizer = TfidfVectorizer()
    try:
        content_vectors = vectorizer.fit_transform(valid_contents)
    except ValueError:
        # Wenn es immer noch ein Problem gibt, gib 0 zurück
        return 0

    # Berechne paarweise Cosine-Similarity
    similarity_matrix = cosine_similarity(content_vectors)

    # Berechne die Durchschnittsähnlichkeit (ohne Diagonale)
    num_items = similarity_matrix.shape[0]
    if num_items < 2:
        return 1.0  # Alle Elemente sind identisch oder es gibt nur eine Empfehlung, Diversität = 1

    # Berechne den durchschnittlichen Ähnlichkeitswert der Elemente (ohne Diagonale)
    sum_similarity = similarity_matrix.sum() - num_items  # Diagonale entfernen
    avg_similarity = sum_similarity / (num_items * (num_items - 1))

    # Höhere Diversität entspricht einem niedrigen Durchschnitt der Ähnlichkeit
...
def calculate_explanation_coherence(recommendations_df):
    ...
    # Kombiniere Attribute für bessere Coherence-Bewertung
    ...
    # Entferne leere oder ungültige Zeilen aus `combined_content`
    valid_content = recommendations_df['combined_content'].replace('', pd.NA).dropna()

    # Wenn die kombinierte Spalte leer ist, gib 0 als Coherence-Wert zurück
    if valid_content.empty:
        return 0

    # Konvertiere die validen Inhalte in numerische Vektoren
    vectorizer = TfidfVectorizer()
    try:
    ...
    # Durchschnittliche Ähnlichkeit als Coherence-Wert
